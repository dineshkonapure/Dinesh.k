...
import { isHolidayIST, isWeekend, iso } from "./lib-server/holidays-in.ts";

function prevTradingDay(dateIso: string){
  const [y,m,d] = dateIso.split("-").map(x=>+x);
  let cur = new Date(Date.UTC(y, m-1, d));
  for(let i=0;i<60;i++){ cur = new Date(cur); cur.setUTCDate(cur.getUTCDate()-1); if(!isWeekend(cur) && !isHolidayIST(cur)) return iso(cur); }
  return dateIso;
}
function uniqueIsin(header: string[], rows: string[][]){ const idx = header.findIndex(h => String(h).toUpperCase() === "ISIN"); if(idx < 0) return 0; const s=new Set<string>(); for(const r of rows){ const k=(r[idx]||"").toUpperCase(); if(k) s.add(k); } return s.size; }
function nullCounts(header: string[], rows: string[][]){ const out:Record<string,number>={}; for(let i=0;i<header.length;i++){ let c=0; for(const r of rows){ if(!r[i] || String(r[i]).trim()==="") c++; } out[header[i]||`col${i}`]=c; } return out; }
...
export const handler: Handler = async (event) => {
  ...
  try{
    ...
    const res = {
      currentDate: date, previousDate: prev,
      rowsCurrent: currParsed.rows.length, rowsPrev: prevParsed.rows.length,
