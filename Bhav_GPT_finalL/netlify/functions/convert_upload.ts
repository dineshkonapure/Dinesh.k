...
import { putBytes } from "./_shared/blob";
...
export const handler: Handler = async (event) => {
  ...
  if(!event.isBase64Encoded || !event.body){
    return { statusCode: 400, headers, body: "Expected multipart/form-data" };
  }
  ...
  const prom = new Promise<void>((resolve, reject) => {
    bb.on("file", (name, file, info) => {
      ...
      count++;
      if(count > MAX_FILES){ aborted = true; file.resume(); reject(new Error("Too many files")); return; }
      ...
      file.on("data", (d: Buffer) => {
        total += d.length;
        if(total > MAX_TOTAL){ aborted = true; reject(new Error("Payload too large")); file.resume(); return; }
        chunks.push(d);
      });
      file.on("end", async () => {
        if(aborted) return;
        ...
        await putBytes(key, new Uint8Array(buf), info.mimeType || "application/octet-stream");
        results.push({ key, size: buf.length, type: info.mimeType || "" });
      });
    });
    bb.on("field", () => {});
