// Indian market holidays (NSE) 2024–2026 (primary list; can be extended via override)
export const HOLIDAYS = new Set<string>([
  // 2024 (sample, extend as needed)
...
export function toISTDateOnly(d: Date){
...
export function isHolidayIST(d: Date, overrides: Set<string>|null=null){
  const day = iso(d);
  return (overrides && overrides.has(day)) || HOLIDAYS.has(day);
}
