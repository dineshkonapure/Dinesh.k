...
export function mergeRows({nse=[], bse=[], amfi=[]}){
  ...
  const out = new Map();
  const push = (arr)=>{ for(const r of arr){ if(!r || !r.ISIN) continue; const k=key(r); if(!out.has(k)) out.set(k,r); } };
  push(nse);
  const nseIsins = new Set(nse.map(r=>r.ISIN).filter(Boolean));
  push(bse.filter(r=> r.ISIN && !nseIsins.has(r.ISIN)));
  push(amfi);
